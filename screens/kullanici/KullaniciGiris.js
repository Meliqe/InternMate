...
export default function KullaniciGiris({ navigation }) {
    ...
    const checkUserProfile = async () => {
        ...
        if (user) {
            try {
                const userDocRef = doc(firestore, 'users2', user.uid);
                const userDoc = await getDoc(userDocRef);

                if (userDoc.exists() && userDoc.data().profileCompleted) {
                    navigation.navigate('KullaniciAnasayfa'); // Profil tamamlanmışsa anasayfaya yönlendir
                } else {
                    navigation.navigate('KullaniciProfilDoldurmaSayfasi');
                }
            } catch (error) {
                console.error(error);
            }
    ...
    const handleRegisterPress = () => {
        navigation.navigate('KullaniciKayit');
    };
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            <View style={styles.container}>
                ...
                <Image style={styles.logo} source={require('../../assets/kullanicigiris.png')} />
                <TextInput
                    style={styles.input}
                    placeholder="E-mail"
